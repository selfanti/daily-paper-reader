...
tldr: 元强化学习中的MAML框架依赖内循环收集轨迹并计算梯度，成本高昂。本文提出LA-MAML，利用任务语言指令的嵌入一步调整全局策略参数，完全替代内循环的轨迹采集和梯度更新。在BabyAI基准上，该方法达到与基线相当或更优的性能，同时将每次迭代的墙钟训练时间大幅降低。结果表明语言指令可作为元强化学习中轨迹内循环的高效替代方案。
source: arxiv
...
motivation: MAML内循环的轨迹收集与梯度更新计算开销大，而语言指令天然携带任务特定信号，可作为更简化的替代。
method: 提出LA-MAML，通过学习任务指令的嵌入，一步调整全局策略参数，跳过内循环的轨迹采集与梯度更新。
result: 在BabyAI基准上，LA-MAML性能与基线相当或更优，且每轮迭代的墙钟训练时间显著降低。
conclusion: 语言指令可作为元强化学习中轨迹内循环的有效且高效的替代方案。
---

## 摘要
模型无关元学习（MAML）是一种广泛使用的强化学习（RL）框架，通过学习能够快速适应新任务的全局策略参数来实现高效迁移。MAML训练分为两个循环：内循环将全局参数适应为任务特定参数，外循环评估这些任务特定参数并通过反向传播损失来改进全局参数。传统上，内循环适应通过从任务环境中收集轨迹并对经验期望回报应用梯度更新来执行，这可能是一项代价高昂的操作。我们注意到，外循环驱动了全局参数的实际学习，因此内循环适应机制不必局限于基于梯度的方法。这一观察引出了一个问题：我们能否用更简单的任务特定信号替代内循环的轨迹收集和梯度更新？在许多实际场景中，任务自然伴随着语言指令。利用这些指令作为直接的任务特定信号，我们提出了LA-MAML（语言适应MAML），它通过任务指令的学习嵌入在单一步骤中调整全局策略参数，从而修改了内循环，替代了内循环的轨迹收集和基于梯度的更新。在BabyAI基准上的实验表明，与基线相比，LA-MAML在每次迭代的墙上时钟训练时间显著降低的情况下，实现了具有竞争力或更优的性能。这些结果表明，在元强化学习中，语言指令是轨迹内循环适应的有效且高效的替代方案。
